...
export function SelectWatchProviders({
  ...
  return (
    <DrawerModalScroll modalRef={modalRef} onDismiss={() => onSelectedProviders()}>
      <Text style={{ color: colors.textHeading, fontWeight: "500", fontSize: 20, marginBottom: 2 }}>Where To Watch ({watchProviders?.length})</Text>
      ...
      <View style={{ marginTop: 20, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5 }}>
        ...
        <Pressable onPress={() => modalRef.current?.dismiss()} style={{ padding: 10, flex: 1, backgroundColor: colors.primary, borderRadius: 15 }}>
          <Text
            style={{
              color: colors.text,
              fontSize: 16,
    ...
    <DrawerModal modalRef={modalRef} onDismiss={() => onSelectedOption()}>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
        {["popularity", "rating"].map((option) => (
          ...
          >
            <Text
              style={[{ textTransform: "capitalize", color: colors.text, fontWeight: 500 }, selectedOption === option && { color: colors.primary }]}
            >
              {option}
            </Text>
          </Pressable>
